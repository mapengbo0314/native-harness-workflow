...
The doc-system is implemented as a **boilerplate template** in `src/harness/templates/boilerplate/`. When harness-wf mints a new harness (Claude, Gemini, Cursor, etc.), the minting engine:
1. Copies the boilerplate directory to the target project
2. Replaces `.agentic-llm` placeholder with the actual harness directory name (`.claude`, `.gemini`, etc.)
3. Applies platform-specific tool mappings
4. Every generated harness gets a complete doc-system out of the box

**1. Central Manifest Registry** (`.agentic-llm/docs/manifest.json` in template)
- Single source of truth for document state (becomes `.claude/docs/manifest.json`, `.gemini/docs/manifest.json`, etc.)
...
**2. File Organization** (`.agentic-llm/docs/` in template)
```
.agentic-llm/docs/                # Becomes .claude/docs, .gemini/docs, etc. after minting
...
### A. Core Infrastructure Files (Template: `src/harness/templates/boilerplate/.agentic-llm/docs/`)

**1. `src/harness/templates/boilerplate/.agentic-llm/docs/manifest.json` (NEW)**
...
- When minted, becomes `.claude/docs/manifest.json`, `.gemini/docs/manifest.json`, etc.
- **Rationale:** Manifest-driven hooks query this file to determine behavior; queryable state without filesystem scanning

**2. `src/harness/templates/boilerplate/.agentic-llm/docs/README.md` (NEW)**
...
**3. `src/harness/templates/boilerplate/.agentic-llm/docs/proposed/` (NEW DIRECTORY)**
...
**4. `src/harness/templates/boilerplate/.agentic-llm/docs/inprogress/` (NEW DIRECTORY)**
...
**5. `src/harness/templates/boilerplate/.agentic-llm/docs/completed/` (NEW DIRECTORY)**
...
**6. `src/harness/templates/boilerplate/.agentic-llm/docs/reference/` (NEW DIRECTORY)**
...
### B. Hook Integration (Template: `src/harness/templates/boilerplate/.agentic-llm/hooks/`)

**7. `src/harness/templates/boilerplate/.agentic-llm/hooks/doc-manifest-validator.sh` (NEW)**
...
**8. `src/harness/templates/boilerplate/.agentic-llm/hooks/doc-state-router.sh` (NEW)**
...
### C. Agent Context Updates (Template: `src/harness/templates/boilerplate/.agentic-llm/agents/`)

**9. `src/harness/templates/boilerplate/.agentic-llm/agents/planner.md` (MODIFY)**
- Add instruction: "Before creating a new design, check `.agentic-llm/docs/manifest.json` for existing or similar designs"
...
**10. `src/harness/templates/boilerplate/.agentic-llm/agents/implementer.md` (MODIFY)**
...
- Add instruction: "Create `.agentic-llm/docs/inprogress/{design_name}-progress.md` mirroring design structure"
...
**11. `src/harness/templates/boilerplate/.agentic-llm/agents/verifier.md` (MODIFY)**
...
**12. `src/harness/templates/boilerplate/.agentic-llm/settings.json` (MODIFY)**
- Add `doc_system` config section:
  ```json
  "doc_system": {
    "enabled": true,
    "manifest_path": ".agentic-llm/docs/manifest.json",
    "hooks_dir": ".agentic-llm/hooks"
  }
  ```
- Add to `pre_commit_hooks`: `"doc-manifest-validator.sh"`
- When minted, `.agentic-llm` becomes `.claude`, `.gemini`, etc.
...
**13. `src/harness/templates/boilerplate/.agentic-llm/hooks/hooks.json` (MODIFY)**
...
**After Minting:** `.claude/`, `.gemini/`, `.cursor/`, etc. (depending on harness type)

| Template Path | Type | Purpose | After Minting |
|---------------|------|---------|----------------|
| `.agentic-llm/docs/manifest.json` | NEW | Central registry of doc states | `.claude/docs/manifest.json` |
| `.agentic-llm/docs/README.md` | NEW | System guide and agent instructions | `.claude/docs/README.md` |
| `.agentic-llm/docs/proposed/` | NEW DIR | Staging area for new designs | `.claude/docs/proposed/` |
| `.agentic-llm/docs/inprogress/` | NEW DIR | Active designs + progress docs | `.claude/docs/inprogress/` |
| `.agentic-llm/docs/completed/` | NEW DIR | Completed designs awaiting archival | `.claude/docs/completed/` |
| `.agentic-llm/docs/reference/` | NEW DIR | Historical archive | `.claude/docs/reference/` |
| `.agentic-llm/hooks/doc-manifest-validator.sh` | NEW | Pre-commit hook to validate manifest | `.claude/hooks/doc-manifest-validator.sh` |
| `.agentic-llm/hooks/doc-state-router.sh` | NEW | Queries manifest for routing decisions | `.claude/hooks/doc-state-router.sh` |
| `.agentic-llm/agents/planner.md` | MODIFY | Add manifest integration | `.claude/agents/planner.md` |
| `.agentic-llm/agents/implementer.md` | MODIFY | Add progress tracking | `.claude/agents/implementer.md` |
| `.agentic-llm/agents/verifier.md` | MODIFY | Add completion validation | `.claude/agents/verifier.md` |
| `.agentic-llm/settings.json` | MODIFY | Add doc_system config and hooks | `.claude/settings.json` |
| `.agentic-llm/hooks/hooks.json` | MODIFY | Register doc-system hooks | `.claude/hooks/hooks.json` |
...
1. Create `src/harness/templates/boilerplate/.agentic-llm/docs/` directory structure: `proposed/`, `inprogress/`, `completed/`, `reference/`
2. Create `src/harness/templates/boilerplate/.agentic-llm/docs/manifest.json` with empty array `[]`
3. Create `src/harness/templates/boilerplate/.agentic-llm/docs/README.md` with system explanation

**Phase 2: Create Hook Scripts**
4. Create `src/harness/templates/boilerplate/.agentic-llm/hooks/doc-manifest-validator.sh` for validation
5. Create `src/harness/templates/boilerplate/.agentic-llm/hooks/doc-state-router.sh` for routing

**Phase 3: Update Agent Templates**
6. Update `src/harness/templates/boilerplate/.agentic-llm/agents/planner.md` with manifest integration
7. Update `src/harness/templates/boilerplate/.agentic-llm/agents/implementer.md` with progress tracking
8. Update `src/harness/templates/boilerplate/.agentic-llm/agents/verifier.md` with validation

**Phase 4: Update Configuration**
9. Update `src/harness/templates/boilerplate/.agentic-llm/settings.json` with doc_system config
10. Update `src/harness/templates/boilerplate/.agentic-llm/hooks/hooks.json` to register doc-system hooks
