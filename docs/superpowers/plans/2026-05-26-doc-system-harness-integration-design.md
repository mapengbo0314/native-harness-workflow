...
2. Replaces `.claude` placeholder with the actual harness directory name (`.claude`, `.gemini`, etc.)
...
- Write paths as `.claude/docs/manifest.json` in boilerplate agent/hook files
- The minting engine automatically replaces `.claude` with the target harness name
- No special layer or placeholder needed; the existing placeholder system handles it

**1. Central Manifest Registry** (`boilerplate/docs/manifest.json` in template)
- Single source of truth for document state (becomes `.claude/docs/manifest.json`, `.gemini/docs/manifest.json`, etc. after minting)
...
**1. `src/harness/templates/boilerplate/docs/manifest.json` (NEW)**
...
**3. `src/harness/templates/boilerplate/docs/proposed/` (NEW DIRECTORY)**
...
**4. `src/harness/templates/boilerplate/docs/inprogress/` (NEW DIRECTORY)**
...
**6. `src/harness/templates/boilerplate/docs/reference/` (NEW DIRECTORY)**
...
|---------------|------|---------|----------------|
| `docs/manifest.json` | NEW | Central registry of doc states | `.claude/docs/manifest.json` |
| `docs/README.md` | NEW | System guide and agent instructions | `.claude/docs/README.md` |
| `docs/proposed/` | NEW DIR | Staging area for new designs | `.claude/docs/proposed/` |
| `docs/inprogress/` | NEW DIR | Active designs + progress docs | `.claude/docs/inprogress/` |
| `docs/completed/` | NEW DIR | Completed designs awaiting archival | `.claude/docs/completed/` |
| `docs/reference/` | NEW DIR | Historical archive | `.claude/docs/reference/` |
| `hooks/doc-manifest-validator.sh` | NEW | Pre-commit hook to validate manifest | `.claude/hooks/doc-manifest-validator.sh` |
| `hooks/doc-state-router.sh` | NEW | Queries manifest for routing decisions | `.claude/hooks/doc-state-router.sh` |
| `agents/planner.md` | MODIFY | Add manifest integration | `.claude/agents/planner.md` |
| `agents/implementer.md` | MODIFY | Add progress tracking | `.claude/agents/implementer.md` |
| `agents/verifier.md` | MODIFY | Add completion validation | `.claude/agents/verifier.md` |
| `settings.json` | MODIFY | Add doc_system config and hooks | `.claude/settings.json` |
| `hooks/hooks.json` | MODIFY | Register doc-system hooks | `.claude/hooks/hooks.json` |
...
2. Create `docs/manifest.json` with empty array `[]`
