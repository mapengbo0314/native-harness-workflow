...
- `<!--$ HARNESS_DIR $-->/docs/designs/`: Stores the primary architectural specifications and problem statements.
- `<!--$ HARNESS_DIR $-->/docs/designs/`: Stores the checklists and task-tracking documents for in-flight implementations.

## Document Lifecycle

### 1. Proposed (Planner Phase)

When a new design is drafted, the Planner creates `<!--$ HARNESS_DIR $-->/docs/designs/{design-name}.md` and places the following YAML frontmatter at the top:
...
2. They create a companion progress tracking document: `<!--$ HARNESS_DIR $-->/docs/designs/{design-name}-progress.md`.
...
1. Updates the frontmatter in `<!--$ HARNESS_DIR $-->/docs/designs/{design-name}.md` to `Status: Completed`.
2. Updates the frontmatter in `<!--$ HARNESS_DIR $-->/docs/designs/{design-name}-progress.md` to `Status: Completed`.
