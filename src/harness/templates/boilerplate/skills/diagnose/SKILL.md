...
When exploring the codebase, use the `codegraph` MCP server and project domain glossary to get a clear mental model of the relevant modules. Call the MCP tool (codegraph\_\*) to gather precise context instead of reading full files, unless absolutely necessary (e.g., using grep_search for UI strings).
...
1. Dispatch `<!--$ subagent('debugger') $-->` to investigate the bug and produce `<!--$HARNESS_DIR$-->/docs/designs/diagnosis_report.md`.
2. Once the debugger completes, dispatch `<!--$ subagent('planner') $-->` to consume the diagnosis report and create a fix plan.
...
Once the root cause is isolated, **DO NOT FIX IT**. You must write your findings to `<!--$HARNESS_DIR$-->/docs/designs/diagnosis_report.md`. The report MUST include:
...
The `@planner` consumes `<!--$HARNESS_DIR$-->/docs/designs/diagnosis_report.md`.
