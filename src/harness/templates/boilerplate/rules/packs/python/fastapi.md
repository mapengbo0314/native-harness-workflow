---
paths:
  - "**/app/**/*.py"
  - "**/fastapi/**/*.py"
  - "**/*_api.py"
---
<!-- ported from affaan-m/ECC@c888d2b rules/python/fastapi.md (curated) -->

# FastAPI Rules

## Structure

- Put app construction in `create_app()`.
- Keep routers thin; move persistence and business logic into services or CRUD helpers.
- Keep request schemas, update schemas, and response schemas separate.
- Inject database sessions and auth via `Depends`.

## Async

- Use `async def` for endpoints that perform I/O.
- Use async database and HTTP clients from async endpoints.
- Do NOT call `requests`, sync SQLAlchemy sessions, or blocking file/network ops from async routes.

## Dependency Injection

```python
@router.get("/users/{user_id}")
async def get_user(
    user_id: str,
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    ...
```

Do not create `SessionLocal()` or long-lived clients inside route handlers.

## Schemas

- Never include passwords, password hashes, or tokens in response models.
- Use `response_model` on endpoints that return application data.
- Use Pydantic field constraints instead of hand-written validation.

## Security

- Keep CORS origins environment-specific; never combine wildcard origins with credentials.
- Validate JWT expiry, issuer, audience, and algorithm.
- Rate-limit auth and write-heavy endpoints.
- Redact credentials and tokens from logs.

## Testing

- Override the exact dependency used by `Depends`.
- Clear `app.dependency_overrides` after each test.
- Prefer async test clients for async applications.
