...
_Conditional Requirement: ONLY required if you are reviewing a tracked task that originated from a design document. If no design/progress doc is associated, skip this section._
...
- **On PASS**: Append your final review checklist to the progress doc, and update the `Status` in both `<!--$HARNESS_DIR$-->/docs/designs/{design_name}.md` and `<!--$HARNESS_DIR$-->/docs/designs/{design_name}-progress.md` to `Completed`.
