...
echo '{"prompt": "Fix the typo in README", "cwd": "'$(pwd)'"}' | python .claude/harness-wf-plugin/hooks/prompt_classifier.py

echo -e "\n3. Testing Branch B (Feature Request)"
echo '{"prompt": "Implement a new authentication system", "cwd": "'$(pwd)'"}' | python .claude/harness-wf-plugin/hooks/prompt_classifier.py

echo -e "\n4. Testing Fallback Logic"
# Unset keys to force failure in the LLM dispatcher
env -u GEMINI_API_KEY -u ANTHROPIC_API_KEY -u OPENAI_API_KEY echo '{"prompt": "There is a bug in the code", "cwd": "'$(pwd)'"}' | python .claude/harness-wf-plugin/hooks/prompt_classifier.py
