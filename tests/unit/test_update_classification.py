...
# ---------------------------------------------------------------------------
# Increment 5: MCP delivery ownership (.mcp.json generated; catalog templated)
# ---------------------------------------------------------------------------


def test_mcp_json_is_emitted_generated():
    """.mcp.json is generated by compile_mcp_config (emitted; no single source)."""
    o = classify(".mcp.json")
    assert o is not None
    assert o.cls == "generated"
    assert o.producer == "emitted"
...
def test_mcp_servers_catalog_is_customizable_template():
    """mcp_servers.json is the operator-editable MCP catalog (like features.yaml)."""
    o = classify("mcp_servers.json")
    assert o is not None
    assert o.cls == "customizable"
    assert o.producer == "template"
    assert o.source_rel == "templates/boilerplate/mcp_servers.json"


def test_enumerate_source_producers_discovers_mcp_catalog(tmp_path):
    """enumerate_source_producers includes mcp_servers.json when the template exists."""
    package_root = tmp_path / "pkg"
    (package_root / "templates" / "boilerplate").mkdir(parents=True)
    (package_root / "templates" / "boilerplate" / "mcp_servers.json").write_text(
        '{"mcpServers": {}}'
    )

    producers = enumerate_source_producers(package_root)

    assert "mcp_servers.json" in producers
    assert producers["mcp_servers.json"].producer == "template"
