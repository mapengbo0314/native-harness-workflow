...
function findPluginDir(
  localPath: string,
): { dir: string; pluginDir: string } | null {
  ...
  try {
    for (const entry of fs.readdirSync(claudeDir, { withFileTypes: true })) {
      ...
      if (fs.existsSync(manifestPath))
        return { dir: entry.name, pluginDir: path.join(claudeDir, entry.name) };
    }
  } catch {
    /* ignore */
  }
...
function getHarnessStatus(localPath?: string): {
  ...
  try {
    ...
    const manifest = JSON.parse(
      fs.readFileSync(
        path.join(found.pluginDir, ".claude-plugin", "plugin.json"),
        "utf8",
      ),
    );
  ...
  try {
    ...
    const results = await Promise.allSettled(
      repos.map(async ({ org, repo, display_name, local_path }) => {
        ...
        const scanState = db
          .prepare(
            "SELECT status, scanned_commits, total_commits, last_scanned_at FROM scan_state WHERE org=? AND repo=?",
          )
          .get(org, repo) as
          | {
              status: string;
        ...
        const churns = (
          db
            .prepare(
              "SELECT additions+deletions AS churn FROM commits WHERE org=? AND repo=? ORDER BY churn",
            )
            .all(org, repo) as { churn: number }[]
        ).map((r) => r.churn);
        const p50 = churns.length
          ? churns[Math.floor(churns.length * 0.5)]
          : null;
        const p90 = churns.length
          ? churns[Math.floor(churns.length * 0.9)]
          : null;

        // Rework rate from cache
        const reworkState = db
          .prepare(
            "SELECT status FROM scan_state WHERE org=? AND repo=? AND status='done'",
          )
        ...
        return {
          ...
          scan_progress:
            scanState?.status === "scanning"
              ? {
                  done: scanState.scanned_commits,
                  total: scanState.total_commits,
                }
              : null,
    ...
    const data = results.map((r, i) =>
      r.status === "fulfilled"
        ? r.value
        : { ...repos[i], error: (r.reason as Error).message },
    );

    res.json(data);
  } catch (error: any) {
    console.error("Error fetching repos list:", error);
