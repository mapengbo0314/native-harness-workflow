  ...
  try {
    ...
    if (!repoConfig) {
      return res
        .status(404)
        .json({
          error: `Repository ${org}/${repo} not configured in repos.yaml`,
        });
    }
    ...
    const [activity, openPrs, recentPrs, codeHealth] = await Promise.all([
      ...
      repoConfig?.local_path
        ? getCodeHealth(org, repo, repoConfig.local_path)
    ...
    const aiCoauthor = hasCache
      ? computeAiCoauthor(org, repo, aiPatterns)
      : null;

    // PR cycle time summary
    const cycle_times = recentPrs
      .map((p) => p.cycle_time_hours)
    ...
    console.error(
      `Error fetching repo ${req.params.org}/${req.params.repo}:`,
      error,
    );
