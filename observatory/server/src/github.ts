...
export async function fetchRecentPrs(org: string, repo: string): Promise<PrSummary[]> {
  const gh = getOctokit()
  ...
  for await (const response of gh.paginate.iterator(gh.pulls.list, {
    ...
    for (const pr of response.data) {
      ...
      prs.push({
        ...
        additions: 0,
        deletions: 0,
        changed_files: (pr as unknown as { changed_files?: number }).changed_files ?? 0,
...
export async function fetchOpenPrCount(org: string, repo: string): Promise<number> {
  const gh = getOctokit()
  let count = 0
  for await (const response of gh.paginate.iterator(gh.pulls.list, {
...
export async function fetchRecentActivity(
  ...
  for (let i = 0; i < weeks; i++) {
    buckets.set(weekKey(new Date(now - i * 7 * 24 * 60 * 60 * 1000).toISOString()), 0)
  }

  const authorCounts = new Map<string, number>()
  const dirtyMap = new Map<string, { raw_name: string; email: string; count: number }>()
  ...
  try {
    for await (const response of gh.paginate.iterator(gh.repos.listCommits, {
      owner: org, repo, since, per_page: 100,
    })) {
      for (const c of response.data) {
        ...
        if (!canonical) {
          ...
          dirtyMap.set(key, { raw_name: rawName, email, count: (existing?.count ?? 0) + 1 })
        }
  ...
  const dirty_authors = [...dirtyMap.values()].sort((a, b) => b.count - a.count)

  return { weekly_counts, last_commit_date: lastDate, last_commit_message: lastMessage, last_commit_author: lastAuthor, top_authors, ai_commit_count: aiCount, total_commit_count: total, dirty_authors }
}

/** Check which agent config files exist in the repo root. */
export async function fetchAgentFiles(org: string, repo: string): Promise<{ claude_md: boolean; agents_md: boolean }> {
  const gh = getOctokit()
  ...
  const [claude_md, agents_md] = await Promise.all([check('CLAUDE.md'), check('AGENTS.md')])
