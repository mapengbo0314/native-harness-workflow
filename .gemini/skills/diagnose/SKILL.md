...
1. Dispatch `@debugger` to investigate the bug and produce `.gemini/docs/designs/diagnosis_report.md`.
2. Once the debugger completes, dispatch `@planner` to consume the diagnosis report and create a fix plan.
...
Once the root cause is isolated, **DO NOT FIX IT**. You must write your findings to `.gemini/docs/reference/diagnosis_report.md`. The report MUST include:
...
The `@planner` consumes `.gemini/docs/reference/diagnosis_report.md`.
