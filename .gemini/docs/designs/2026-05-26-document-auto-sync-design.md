...
The system currently tracks design documents across a four-stage lifecycle (`proposed`, `inprogress`, `completed`, `reference`). However, when an implementation fails during the `inprogress` phase, agents are instructed to write a standalone `_failure_report.md` file directly into the `.gemini/docs/reference/` directory.
...
1. **Agent Responsibility**: Agents will only edit `.gemini/docs/manifest.json` to change a document's state. 
...
1. **`src/harness/templates/boilerplate/scripts/sync_manifest_state.py` (NEW):**
   - *Rationale:* A Python script that reads `.gemini/docs/manifest.json`. It checks the `state` of every document and compares it to its physical location. If a document labeled `inprogress` is still sitting in `.gemini/docs/designs/`, the script executes a `git mv .gemini/docs/designs/{name}.md .gemini/docs/designs/{name}.md` (and similarly for `completed/` transitions). This centralizes the movement logic.
