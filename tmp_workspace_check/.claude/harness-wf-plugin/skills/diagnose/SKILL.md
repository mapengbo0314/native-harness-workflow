...
When exploring the codebase, use the `codegraph` MCP server and project domain glossary to get a clear mental model of the relevant modules. Call the MCP tool (codegraph\_\*) to gather precise context instead of reading full files, unless absolutely necessary (e.g., using Grep for UI strings).

## Checklist

- Run the extraction hook: `python .claude/scripts/extract_stacktrace.py <logfile>` to isolate the error.
...
1. Dispatch `Task(subagent_type="debugger")` to investigate the bug and produce `.claude/docs/designs/diagnosis_report.md`.
2. Once the debugger completes, dispatch `Task(subagent_type="planner")` to consume the diagnosis report and create a fix plan.
...
Once the root cause is isolated, **DO NOT FIX IT**. You must write your findings to `.claude/docs/designs/diagnosis_report.md`. The report MUST include:
...
The `@planner` consumes `.claude/docs/designs/diagnosis_report.md`.
