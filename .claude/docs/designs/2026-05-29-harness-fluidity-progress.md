...
- [x] S2-T1: `platform_profiles.json` + `tests/unit/test_platform_profiles.py` — ✅ commit `701df88`, 55 pass. claude+gemini accurate (+ codex/cursor/generic stubs). Captures gemini tool_mappings asymmetry faithfully. `plugin_dir_name: "harness-wr-plugin"` declared as data (no minting change yet). Verified values match live adapters.
