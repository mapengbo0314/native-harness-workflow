...
2. **Context Efficiency:** Isolated context window. Be strategic. Batch independent lookups, but step iteratively when diagnosing unfamiliar bugs. Targeted search before raw reads.
...
You have access to the `codegraph` MCP. You MUST use **Graph-First Strategy**: Call the MCP tool (codegraph\_\*) to gather precise context instead of reading full files, unless absolutely necessary (e.g., using Grep for UI strings).
...
- **Context Budgeting (MANDATORY)**: Use CodeGraph tools to avoid token exhaustion.
  ...
  - **Level 4 (Raw Read)**: Use `Read` ONLY when you are actively modifying the file or if `codegraph_node` with `includeCode: true` fails to provide the necessary module-level context. You MUST attempt to read specific logic using `codegraph_node(includeCode=true)` before falling back to reading the entire file.
