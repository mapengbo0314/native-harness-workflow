...
- Save the final document to `.claude/docs/designs/YYYY-MM-DD-<topic>-design.md` (or the user's preferred spec location).
- Initialize a corresponding `.claude/docs/designs/YYYY-MM-DD-<topic>-progress.md` file with the extracted tasks.
...
```bash
python3 "$CLAUDE_PLUGIN_ROOT/scripts/check_risk_report.py" ".claude/docs/designs/YYYY-MM-DD-<topic>-design.md"
```
...
```bash
python3 "$CLAUDE_PLUGIN_ROOT/scripts/session_phase.py" clear-phase --session "<id from SYSTEM STATE>" --artifact ".claude/docs/designs/YYYY-MM-DD-<topic>-design.md"
```
