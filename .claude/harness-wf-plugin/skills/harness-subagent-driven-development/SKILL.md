...
DO NOT map to generic `Task(subagent_type="generalist")` agents. You MUST map tasks to the specialized local harness subagents:

- For planning and specs: `Task(subagent_type="planner")`
- For writing code: `Task(subagent_type="implementer")`
- For QA/Review: `Task(subagent_type="reviewer")` or `Task(subagent_type="verifier")`
...
digraph when_to_use {
    ...
    "harness-subagent-driven-development" [shape=box];
    ...
    "Stay in this session?" -> "harness-subagent-driven-development" [label="yes"];
...
- Let implementer self-review Edit actual review (both are needed)
...
- **superpowers:harness-test-driven-development** - Subagents follow TDD for each task
