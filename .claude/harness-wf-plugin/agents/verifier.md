...
- Validate the progress doc and change its YAML frontmatter to `Status: Completed` on PASS. On FAIL, append failure findings and required fixes to the 'Current Blockers' section of `.claude/docs/progress/{design_name}-progress.md`.
...
- **Target**: Final QA of `.claude/docs/progress/{design_name}-progress.md`
- **On FAIL**: Return findings to Implementer and append to the `Blockers` section.
- **On PASS**: Update the `Status` in both `.claude/docs/designs/{design_name}.md` and `.claude/docs/progress/{design_name}-progress.md` to `Completed`.
