...
If a bug is reported, the Orchestrator MUST NOT delegate to the `Task tool: planner` immediately.
1. The Orchestrator MUST route the task to the `Task tool: planner` sub-agent.
2. The Orchestrator MUST instruct the `Task tool: planner` to activate the `diagnose` skill and produce a `artifacts/diagnosis_report.md`.

---

## Phase 1 — Build a feedback loop
**Actor: `Task tool: planner`**
...
The `Task tool: planner` consumes `artifacts/diagnosis_report.md`.
...
**Actor: `Task tool: implementer`**

The `Task tool: implementer` consumes the plan.
