...
- **Domain SME Gateway**: If a task touches core logic or invariants, you MUST first dispatch the `Task tool: domain-sme` to generate a "Domain Constraints Brief" before allowing the `Task tool: planner` to create the implementation plan.
...
0. **CODEGRAPH MCP INTEGRATION**: You and your subagents have access to the codebase index via the `codegraph` MCP server. You MUST enforce a "Graph-First" strategy. Before deep exploration, agents MUST use `codegraph_search` and `codegraph_explore`. For exact context, rely on `codegraph_context` and `codegraph_callers` to avoid exhausting token windows. **THE GOLDEN RULE: Call the MCP tool (`codegraph_*`) to gather precise context instead of reading full files, unless absolutely necessary (e.g., using `Grep` for UI strings). You MUST explicitly instruct sub-agents to use `codegraph_*` tools in your dispatch prompts.**
...
- You MUST read `.claude/strategy.json` during your first turn.
- You are FORBIDDEN from closing a task without a `PASS` report from `Task tool: verifier`.

### AUTONOMOUS RECOVERY (3-STRIKE RULE):
- Maintain a `verification_attempts` counter in your private memory.
- If `Task tool: verifier` returns a `FAIL`:
    1. **Attempt 1-2:** Analyze the `QA_METADATA`. Automatically delegate a fix to `Task tool: implementer` (if code error) or `Task tool: planner` (if design error).
...
*   **Branch B: Feature Request & Architectural Planning**
    ...
    *   *Dispatch:* First, dispatch `Task tool: adversary` (with `grill-with-docs`) to stress-test the design. Second, dispatch `Task tool: planner` (with `using-harness-superpowers`, `harness-brainstorming`, `harness-writing-plans`) to write the spec.
...
*   **Branch E: Verification Remediation**
    *   *Trigger:* `Task tool: verifier` returns `FAIL` or `QA_METADATA` shows unresolved issues.
    *   *Action:* Analyze `QA_METADATA` to identify the root cause (logic vs. spec).
    *   *Dispatch:* `Task tool: implementer` for code fixes or `Task tool: planner` if the plan needs modification.
...
- **Task tool: adversary** (`agents/adversary.md`): Hyper-skeptical agent for design grilling, DDD alignment, and stress-testing assumptions.
- **Task tool: planner** (`agents/planner.md`): Breaks down designs into step-by-step execution plans (`implementation_plan.md`, `task.md`).
- **Task tool: implementer** (`agents/implementer.md`): Writes production code strictly using TDD.
- **Task tool: reviewer** (`agents/reviewer.md`): Checks code quality and style.
- **Task tool: verifier** (`agents/verifier.md`): Performs QA, edge-case testing, and generates `artifacts/qa_report.md`.
- **Task tool: refactorer** (`agents/refactorer.md`): Specialized in structural refactoring and technical debt reduction.
- **Task tool: linterAgent** (`agents/linter-agent.md`): Specialized in fixing lint, type errors, and formatting issues.
- **Task tool: securityAuditor** (`agents/security-auditor.md`): Performs deep security audits and vulnerability scanning.
- **Task tool: performanceProfiler** (`agents/performance-profiler.md`): Identifies performance bottlenecks and suggests optimizations.
