...
When exploring the codebase, use the `codegraph` MCP server and project domain glossary to get a clear mental model of the relevant modules. Call the MCP tool (codegraph_*) to gather precise context instead of reading full files, unless absolutely necessary (e.g., using Grep for UI strings). 
...
2. The Orchestrator MUST instruct the `@planner` to activate the `diagnose` skill and produce a `.claude/docs/reference/diagnosis_report.md`.
...
Once the root cause is isolated, **DO NOT FIX IT**. You must write your findings to `.claude/docs/reference/diagnosis_report.md`. The report MUST include:
...
The `@planner` consumes `.claude/docs/reference/diagnosis_report.md`.
